...
colorSelect.addEventListener("change", () => setGradient(colorSelect.value));
...
void main() {
  fragColor = vec4((v_color0 + v_color1 + v_color2) / 3.0, 1.0);
}`;
...
const color2Loc = gl.getAttribLocation(program, "a_color2");
...
function initTriangles(count) {
  ...
  colors1 = new Float32Array(count * 3);
  colors2 = new Float32Array(count * 3);
  ...
  for (let i = 0; i < count; i++) {
    ...
    colors1.set(randomColor(grad), i * 3);
    colors2.set(randomColor(grad), i * 3);
  }
...
function render(time) {
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer2);
  gl.vertexAttribPointer(color2Loc, 3, gl.FLOAT, false, 0, 0);
  gl.enableVertexAttribArray(color2Loc);
  gl.vertexAttribDivisor(color2Loc, 1);
